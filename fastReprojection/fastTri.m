%this is my attempt at speeding up the triangulate code from DLR
%NOTE: no speed improvements made so far...

%und_upper = undistorted points for cam 1
%und_lower = undistorted points for cam 2
%A = camera 1 matrix
%A_stereo = camera 2 matrix
%T_cam1_cam2 = rotation and translation between cameras
function point_camera = fastTri(und_upper, und_lower, A, A_stereo, T_cam1_cam2)
    ...
    euclidean_error = sqrt(sum(error(1:3).^2));
     %point_camera(4)=euclidean_error;
    if euclidean_error>1000
